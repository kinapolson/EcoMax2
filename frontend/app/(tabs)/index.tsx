...
export default function HomeScreen() {
  const { name, lastName, email, points, userId } = useLocalSearchParams();
  ...
  useEffect(() => {
    if (userId) {
      setUser({
        ...
        firstName: String(Array.isArray(name) ? name[0] : name ?? ''),
        lastName: String(Array.isArray(lastName) ? lastName[0] : lastName ?? ''),
        email: String(Array.isArray(email) ? email[0] : email ?? ''),
        ecoPoints: String(Array.isArray(points) ? points[0] : points ?? ''),
      });
    }
  }, [userId]);
