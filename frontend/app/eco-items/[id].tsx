...
export default function BusinessDetails() {
  ...
  useEffect(() => {
    fetch(`http://192.168.1.157:8000/get_b_items.php?item_id=${id}`)
      .then(res => res.json())
      .then(data => {
        if (data.status === "success") {
          setItem(data.item);
          fetch(`http://192.168.1.157:8000/get_b_items.php?b_id=${data.item.b_id}`)
            .then(res => res.json())
            .then(data2 => {
          ...
          <View style={styles.itemsGrid}>
            {similarItems.map((business) => (
              ...
              >
                ...
                <Image
                  source={{ uri: `http://192.168.1.157:8000/items/${business.image}` }}
